...
const rows = projects.map((project) => {
  ...
  return `| ${(project.lifecycle.shareable ? link(project.public?.name ?? project.name, profileUrl) : escapeCell(project.public?.name ?? project.name))} | ${escapeCell(contract.purpose)} | ${escapeCell(project.lifecycle.status)} | ${link('Product', destinationUrl)} | ${sourceLink} | ${escapeCell(contract.nextAction)} |`;
});

const rendered = `# Fleet product-purpose contracts\n\nGenerated from \`apps/backend/config/projects.json\` on 2026-09-07. This is the reconciliation view for the ${projects.length} retained product identities; the shared \`ios-landings\` factory is intentionally excluded. Repository-local \`PRODUCT.md\` or \`PROJECT_STATUS.md\` wins when it contains newer product truth.\n\nAll ${projects.length} contracts currently have a resolved purpose, audience, outcome, mechanism, proof, and lifecycle-honest next action. Shareability and removal decisions are tracked separately in the canonical lifecycle and current verification report. SaaS Maker links identify the intended public profile routes; the new contract UI remains local until a separately authorized deployment.\n\n| Project profile | Canonical purpose | Lifecycle | Destination | Repository truth | Honest next action |\n| --- | --- | --- | --- | --- | --- |\n${rows.join('\n')}\n`;
