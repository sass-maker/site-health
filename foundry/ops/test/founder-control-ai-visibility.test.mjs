...
test('every maintained public Fleet identity has product-specific fixture coverage', () => {
  ...
  assert.equal(portfolio.eligible.length, 31);
...
test('provider observation all-project gate requires the exact canonical 31', () => {
  const portfolio = loadAiVisibilityPortfolio();
  assert.equal(portfolio.eligible.length, 31);
  assert.throws(
    () => prepareProviderObservationRuns({
  ...
  assert.equal(prepared.length, 31);
});
