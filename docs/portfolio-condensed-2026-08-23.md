...
| Forecast Lab | Inactive | Learn forecasting through reproducible experiments. | Retained experiment; the owner explicitly does not require sharing or a release. | Preserve the bounded learning example and source; no adoption or license-choice task is required for retention. |
...
| Reel Pipeline | Inactive | Explore automated product-video creation. | Retained experiment; the owner explicitly does not require sharing or a release. | Preserve the bounded render evidence and source; resume only for a deliberate video experiment. |
