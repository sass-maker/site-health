...
```json
{
  ...
  "jobsFile": "/path/to/foundry-checkout/fleet-ops/automation/codex-cron/jobs.tsv",
  "systemJobsFile": "/path/to/foundry-checkout/fleet-ops/automation/codex-cron/system-jobs.tsv",
  "codexRunner": "/path/to/foundry-checkout/fleet-ops/scripts/agent-bin/run-codex-cron",
  "systemRunner": "/path/to/foundry-checkout/fleet-ops/scripts/agent-bin/run-system-cron",
...
node fleet-ops/host/hostctl.mjs render --role-file "$ROLE_FILE"
node fleet-ops/host/hostctl.mjs dry-run --role-file "$ROLE_FILE"
node fleet-ops/host/hostctl.mjs promote --role-file "$ROLE_FILE" --ttl-seconds 900
node fleet-ops/host/hostctl.mjs status --role-file "$ROLE_FILE"
node fleet-ops/host/hostctl.mjs pause --role-file "$ROLE_FILE"
node fleet-ops/host/hostctl.mjs resume --role-file "$ROLE_FILE" --ttl-seconds 900
node fleet-ops/host/hostctl.mjs revoke --role-file "$ROLE_FILE"
```
...
`fleet-ops/config/performance-schedules.json` is checked-in intent, not an installed
