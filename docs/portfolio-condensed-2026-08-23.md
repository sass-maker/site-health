...
| High Signal Podcasts (`on-record`) | Active | Index what notable people said on podcasts as evidenced claims, including books/apps they use. | Live at podcasts.highsignal.app as a High Signal sub-product with its own layout; published corpus is still a thin V1 slice. | Grow the evidenced recs/stack index and keep the public listing hidden until the corpus is worth sharing. |
...
| IssuePages (`issue-pages`) | Active | Make publishing a public page as simple as opening a GitHub issue. | Owner-only production pilot is live; the issue/edit/archive/search/render loop is proven, while public moderation and authenticated GitHub rendering remain gated. | Connect the GitHub render token, then OpenAI moderation; study whether strangers publish and readers continue browsing. |
