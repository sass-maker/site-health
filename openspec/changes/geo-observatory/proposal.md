...
1. **Query config** (`fleet-ops/config/geo-observatory.json`): per tracked
   product, the exact brand/category/citation queries to probe, stable
   across runs so results are comparable.
2. **Observation protocol** (`fleet-ops/skills/geo-observatory/SKILL.md`):
...
3. **Ledger + report** (`fleet-ops/scripts/geo-observatory-record.mjs`):
   appends validated observations to
   `fleet-ops/data/geo-observatory/ledger.jsonl` and regenerates
   `fleet-ops/docs/geo-observatory-latest.md` — per-product trend table
