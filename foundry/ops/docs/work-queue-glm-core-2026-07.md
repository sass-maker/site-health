...
4. **Follow** the repo's `AGENTS.md`, `LANDING_STANDARD.md` (marketing), and
   `foundry/ops/docs/agent-indexing-standard.md` (machine surfaces).
5. **Verify machine surfaces** against `foundry/ops/skills/agent-ready/scripts/agent-index-audit.mjs`
   after edits.
...
- I design the methodology + harness before any execution. glm's role will be
  ...
  SEO article.**
- **Design (done):** `foundry/ops/docs/proof-assets/codevetter-competitor-benchmark.md`.
...
- **Design (done):** `foundry/ops/docs/proof-assets/heypace-ondevice-benchmark.md`.
...
- Registry entry: `foundry/ops/config/agent-surfaces-registry.json` (`posttrainllm`
...
- **Copy (done):** `foundry/ops/docs/outreach-evidence-packages-2026-07.md` —
...
`foundry/ops/config/geo-observatory.json` has only 2 queries per core product.
- Add the comparison + how-to queries from the query-universe map for CodeVetter,
  ...
  baseline via `foundry/ops/scripts/geo-observatory-record.mjs`.
