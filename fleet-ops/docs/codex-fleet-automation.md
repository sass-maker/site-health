...
set -euo pipefail
FLEET=~/Desktop/fleet
EXCLUDE="port-whisperer|local-ai|free-ai"   # out-of-fleet + hands-off (see AGENTS.md)
...
specific jobs to `fix`. Excludes `free-ai` (hands-off), `port-whisperer`, `local-ai`
(out-of-fleet) per [`AGENTS.md`](../../AGENTS.md).
