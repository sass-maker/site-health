...
You have access to **psi-swarm** — a CLI tool that runs Lighthouse many times against a URL across realistic device/network presets, then explains the results. It lives in the fleet-ops project at `foundry/apps/internal/psi-swarm/` (this skill file is its canonical home).
...
psi-swarm lives in the fleet workspace at `~/Desktop/fleet/foundry/apps/internal/psi-swarm/`.
The CLI binary is `node ~/Desktop/fleet/foundry/apps/internal/psi-swarm/cli/dist/cli.js`.

If `cli/dist/cli.js` is missing, build it once:

```bash
cd ~/Desktop/fleet/foundry/apps/internal/psi-swarm && pnpm install && pnpm --workspace cli run build
```
