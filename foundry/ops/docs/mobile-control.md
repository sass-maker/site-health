...
./foundry/ops/scripts/agent-bin/mobile-control configure-telegram
```
...
./foundry/ops/scripts/agent-bin/create-teammate ship \
  --board shipping \
  --mention @shipbot \
...
```sh
./foundry/ops/scripts/agent-stack.sh resume
./foundry/ops/scripts/agent-bin/mobile-control status
./foundry/ops/scripts/agent-bin/mobile-control start-tailscale
./foundry/ops/scripts/agent-bin/mobile-control serve-dashboard
./foundry/ops/scripts/agent-bin/mobile-control ping "Fleet is online"
```

Emergency only, after accepting the credential risk:

```sh
./foundry/ops/scripts/agent-bin/mobile-control emergency-tmate --i-understand
```
...
1. Clone `sass-maker/fleet-workspace` into `~/Desktop/fleet`; Fleet Ops lives
   in its `foundry/ops/` subdirectory.
2. Run `./foundry/ops/scripts/agent-stack.sh install-skills`.
3. Run `./foundry/ops/scripts/agent-bin/mobile-control start-tailscale`.
