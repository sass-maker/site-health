...
| IndexNow tooling | `foundry/ops/scripts/indexnow-submit.mjs` + docs |
...
| Audit prompt | `foundry/ops/docs/audit-prompt-geo-seo-session.md` |
| Strategy docs | `geo-dr-outcomes.md`, `indexnow.md`, directory-submissions demoted spam for DR |

## IndexNow key

Public key (by design): see `foundry/ops/config/indexnow.json` → `key`
...
node foundry/ops/scripts/indexnow-submit.mjs --dry-run --max 20
