...
Fleet skills are exposed via 4 parents + standalones (canonical home:
`foundry/ops/skills/` — never add skills to `~/.claude/skills`):
...
| `design-engineering` | parent | inspiration, component pattern, web 3D, and creative-effects subskills |
...
teammates. The `call-teammate` parent skill (at `foundry/ops/teammates/skills/call-teammate/`)
routes to the right subskill based on which CLI the user names:

- Subskills: `foundry/ops/teammates/skills/call-codex|call-grok|call-claude-code|call-devin|call-cursor`.
...
- Outcomes: `foundry/ops/teammates/SCORECARD.md` — append one line per delegation;
...
`foundry/ops/teammates/ROSTER.md` and log the switch in the scorecard — don't
...
The fleet-wide audit and per-project scaffold notes live at `foundry/ops/docs/learning-track-audit.md`.
