...
| Devin (`devin -p` / `cloud`) | `call-devin` | **Fire-and-forget cloud sessions** that outlive you; OS-level `--sandbox`; multi-model (`--model codex`) | **ACU-metered** — default elsewhere; no JSON output | ⬜ help-only |
...
- **Implementation / test-fix**: codex → claude-code (work profile →
  personal profile) → grok (`--always-approve` + manual worktree).
- **Review / second opinion**: codex `exec review` → grok → claude-code.
  (Prefer keeping model diversity: if the parent is Claude, exhaust the
  non-Anthropic reviewers first.)
