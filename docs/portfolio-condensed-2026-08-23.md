...
| LaunchDesk (`launchdesk`) | Active | Give makers a provenance-honest catalog of launch destinations with per-product submission tracking. | Live publicly at launchdesk.significanthobbies.com as a browser-storage static Worker app; the live link-checker stays local-only by owner decision. | Keep: The 966-destination attributed catalog with explicit unknowns and browser-local tracking. Stop: Accounts, automated submission, outreach features and invented metrics. |
