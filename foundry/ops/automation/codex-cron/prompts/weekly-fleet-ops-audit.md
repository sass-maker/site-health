...
2. Run `./foundry/ops/scripts/git-health.sh --all --no-fetch`.
3. Run `./foundry/ops/scripts/deploy-health.sh` and clearly separate GitHub, Cloudflare, and manifest limitations.
4. Run `node foundry/ops/scripts/cloudflare-resilience-audit.mjs`; read `.symphony/cloudflare-resilience/latest.md` and `latest.json`.
