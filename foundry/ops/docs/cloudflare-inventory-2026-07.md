...
## Cloudflare Pages projects (17, all live)
...
| **tinygpt** | posttrainllm.com, www | posttrainllm | **Focus** | project remains named `tinygpt` for continuity; no fallback custom domain |
...
1. **`tinygpt` Pages project = PostTrainLLM (Focus).** The CF project is still
   named `tinygpt`; this is cosmetic only. Its canonical custom domains are
   `posttrainllm.com` and `www.posttrainllm.com`; the generated
   `tinygpt.pages.dev` hostname is not a fallback domain.
2. **Orphan Pages with no custom domain:** `saas-ideas` (no repo/registry match
   at all). Delete or claim.
3. **ai-game is live** (`aliveville.com` + `idle.aliveville.com`, 2 Pages) while
   tiered Parked — the tier is fine, but the surfaces are real; not in registry.
4. **everythingrated has two apexes** (`everythingrated.com` +
   `ratings.highsignal.app`); registry lists only the subdomain — pick the
   canonical and 301 the other.
5. **`cloudflare.targets.json` is incomplete** — it maps only 13 products; the
...
1. **PostTrainLLM domain cleanup:** stray `tinygpt.sarthakagrawal.dev`
   **REMOVED** 2026-07-18. `posttrainllm.com` and `www.posttrainllm.com` are the
   only custom domains; the Pages default hostname remains platform-generated,
   not a fallback.
2. **knowledgebase-landing:** DELETED 2026-07-18 via the Cloudflare dashboard.
   The live Knowledgebase Worker/app remains on `search.sassmaker.com`; the
   GitHub repository was not affected. **saas-ideas:** kept (user: let it be).
3. **today-little-log:** DELETED 2026-07-18 via API (purged 126 deployments,
   then the project). Note: `open-historia` **Worker** is still live (out-of-fleet)
...
6. **Canonical manifest built:** `foundry/ops/config/projects.json` (now 34
...
- **Green check:** all 17 Pages `latest=success`; all 30 custom domains 200/3xx.

**Live CF counts (2026-07-18): 17 Pages + 25 Workers = 42 deployment units.**
...
The canonical manifest now exists: **`foundry/ops/config/projects.json`** (33
