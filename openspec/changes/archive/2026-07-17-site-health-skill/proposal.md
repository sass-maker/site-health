...
- New parent skill `fleet-ops/skills/site-health/SKILL.md`:
  ...
  - Adds the missing invocation: **full health check** for one product or
    ...
    one combined scorecard (`fleet-ops/docs/site-health-latest.md`).
- Children converge on shared plumbing:
  - Target resolution via `fleet-ops/scripts/lib/registry.mjs` (landing in
    `fleet-jsonld-emission` task 1 — hard dependency, do not start before
    it merges).
  - One report-location convention: `fleet-ops/docs/<skill>-latest.md`.
