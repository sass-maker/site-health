...
Date: 2026-08-23 (lifecycle counts and new owner-requested identities reconciled 2026-09-25)
Scope: all 72 retained canonical identities; archived identities remain recorded for infrastructure ownership but are excluded from Fleet listings.
...
| Active | 32 | Supported and kept useful; improvements follow concrete needs. |
| Inactive | 38 | No planned development or autonomous expansion. Preserve relevant software, data and evidence. |
| **Total** | **72** | Every canonical Fleet identity appears exactly once. |
...
| --- | --- | --- | --- | --- |
| ContextDaddy (`contextdaddy`) | Active | Inspect local coding-agent context, skills, usage, and available telemetry to debug and speed up work. | Private macOS development app in the Fleet checkout; no signed public release or completed native acceptance. | Qualify the local workflow before considering distribution. |
...
| Fleet Social (`fleet-social`) | Active | Give Fleet projects one private place to draft, review, schedule, and account for social posts. | The private CogSend-based Worker is live with Cloudflare, GitHub, and Google sign-in; the owner YouTube channel connected and one approved MP4 canary published privately with matching Fleet Social and YouTube receipts. | Verify machine draft intake and an actual scheduled publish, then make the Google OAuth connection durable beyond Testing. |
...
| BrowserDaddy (`browserdaddy`) | Active | Build a local-first Mac archive of browsing history and attention across browsers. | Public landing is live; installed 0.3.1 build 4 is notarized and matches the live update feed. Its GitHub release is private and the landing has no first-install download. | Continue native privacy qualification and decide when to expose a supported install path. |
| Meme Lab (`meme-lab`) | Active | Turn a pasted comment or situation into the most relevant meme immediately. | A 300-reference personal playground is live at memes.significanthobbies.com; a separate 1,000-record source pool is under review. | Curate 700 stage-1,000 records, expand the evaluation set, then scale the same retrieval and ranking pipeline toward 3,000 memes. |
| Every Song Is a Website (`every-song-is-a-website`) | Active | Give each famous song its own curated interactive visual world — a playful gallery, not a streaming product. | Live at music.significanthobbies.com as a static Workers-assets deploy; 143 songs across five worlds, 32 of them pulled from the owner's Apple Music library with YouTube and Genius embeds. | Maintain the catalog and worlds; add themes or songs only on explicit request. |
...
| LaunchDesk (`launchdesk`) | Inactive / absorbed into SaaS Maker | Historical source of SaaS Maker's launch-destination catalog; no separate product. | Owner removed the standalone identity on 2026-09-20. The `/launchdesk` feature and checked-in dataset now belong entirely to SaaS Maker. | No independent roadmap or portfolio listing. Any future catalog work belongs to SaaS Maker. |
...
| Nomad Data Adventure (`nomad-data-adventure`) | Completed experiment | Explore and compare places to live using collected city data. | Shareable static Nomad Atlas explorer on Cloudflare Pages: 1,374 terrestrial places, cost and lifestyle filters, detailed comparisons and public JSON credited to Nomads.com. Canonical domain: nomad.significanthobbies.com. | Completed experiment; keep the public product and saved dataset, with no active expansion or collection roadmap. |
...
| PH Catalog (`ph-catalog`) | Active | Build a resumable local Product Hunt catalogue with explicit provenance, verified snapshots and useful aggregate analytics. | Reactivated for one bounded finishing pass; the independent Python/DuckDB repository and local catalogue data remain separate while SaaS Maker owns its canonical portfolio identity and safe public listing. | Finish and document the repeatable local workflow, then mark it inactive as a completed experiment. |
| PerformanceDaddy (`performancedaddy`) | Active | Investigate local Mac performance through visible processes, ports, resource measurements, and reviewed actions. | Public landing is live; the installed notarized 0.2.3 build 3 matches the public GitHub release and live update feed. | Use measured evidence before acting on a diagnosis; keep the landing and native release claims distinct. |
...
| SaaS Maker | Active | House reusable packages, public Fleet discovery, the source-attributed launch-destination catalog, feedback tooling, workflows, skills, and operator automation. | Canonical tooling home; public directory includes only verified shareable surfaces. The former standalone LaunchDesk identity was absorbed here on 2026-09-20 as the native `/launchdesk` feature and checked-in dataset. | Stabilize its package boundaries and let real Fleet consumers determine which packages and catalog capabilities deserve investment; LaunchDesk has no independent roadmap or portfolio listing. |
| Scale (`scale`) | Inactive | Teach systems-engineering capacity and architecture trade-offs through a small idle game. | The standalone prototype is archived; the maintained game lives inside SWE Interview Prep. | Preserve the original experiment and direct future play or maintenance to the SWE Interview Prep implementation. |
...
| StorageDaddy (`storagedaddy`) | Active | Build a native Mac storage analyzer with reviewed cleanup. | Public signed update feed advertises 0.1.2 build 80, matching the installed notarized app; earlier native qualification is bounded to its recorded builds. | Continue measuring scan correctness and runtime efficiency before expanding scope. |
...
| Paths (`what-it-takes-to-win`) | Active | Study advantages behind success and turn comparison into evidence-based understanding. | Strong research publication with clear personal motivation and room for deeper coverage. | Continue studying people, improve source-linked life histories, and reconcile stale public dataset counts during the next refresh. |
...
MentionPilot was explicitly reactivated on 2026-09-14 and registered as a canonical active project on 2026-09-25. Its older archived-work entry below is retained only as the dated 2026-09-10 decision record.
