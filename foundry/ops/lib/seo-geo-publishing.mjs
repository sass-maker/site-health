...
export function validateSeoGeoPublishing(program, catalog) {
  ...
  for (const channel of program?.channels ?? []) {
    ...
    if (!isHttpsUrl(channel.guidanceUrl)) {
      errors.push(`${channel.id}: guidanceUrl must be HTTPS`);
    }
    if (!isDate(channel.reviewedAt)) {
      errors.push(`${channel.id}: reviewedAt must be YYYY-MM-DD`);
    }
  }
  if (channels.size === 0) errors.push('channels must be non-empty');
  const channelExecution = program?.channelExecution ?? {};
  ...
  for (const plan of program?.projects ?? []) {
    ...
    for (const placement of placements) {
      if (!channels.has(placement.channelId)) {
        errors.push(`${plan.projectId}: unknown placement channel ${placement.channelId}`);
      }
      if (placementChannels.has(placement.channelId)) {
        errors.push(`${plan.projectId}: duplicate placement channel ${placement.channelId}`);
      }
      placementChannels.add(placement.channelId);
      if (!RANKS.has(placement.rank)) {
        errors.push(`${plan.projectId}: invalid placement rank ${placement.rank}`);
      }
      if (!ACTORS.has(placement.actor)) {
        errors.push(`${plan.projectId}: invalid placement actor ${placement.actor}`);
      }
      if (!placement.format) errors.push(`${plan.projectId}: placement missing format`);
      if (!placement.fit) errors.push(`${plan.projectId}: placement missing fit`);
    }

    const exclusionChannels = new Set();
    for (const exclusion of plan.exclusions ?? []) {
      if (!channels.has(exclusion.channelId)) {
        errors.push(`${plan.projectId}: unknown exclusion channel ${exclusion.channelId}`);
      }
      if (exclusionChannels.has(exclusion.channelId)) {
        errors.push(`${plan.projectId}: duplicate exclusion channel ${exclusion.channelId}`);
      }
