#!/usr/bin/env node
// Manage the launchd agent that recurs `indexing-requests.mjs check`.
//
// The collector's own cadence (first inspection ~20h after submission, then at
// most every ~20h until indexed or day 14) makes a daily run the right loop.
...
const LABEL = 'com.sarthak.indexing-check';
...
const COLLECTOR = join(REPO_ROOT, 'apps', 'backend', 'scripts', 'indexing-requests.mjs');
...
function resolveSchedule({ hour = 7, minute = 30 }) {
...
<dict>
  ...
  <array>
    ...
    <string>-lc</string>
    <string>cd ${REPO_ROOT} &amp;&amp; exec node ${JSON.stringify(COLLECTOR)} check</string>
  </array>
...
function runStatus() {
  ...
  const lines = [
    ...
    `command    node apps/backend/scripts/indexing-requests.mjs check`,
  ];
  if (installed) {
    const plist = readFileSync(PLIST_PATH, 'utf8');
    const calendar = /<key>StartCalendarInterval<\/key>\s*<dict>([\s\S]*?)<\/dict>/.exec(plist);
    if (calendar) lines.push(`cadence    daily at ${calendar[1].replace(/\s+/g, ' ').trim()} (StartCalendarInterval)`);
  }
...
// `/bin/zsh -lc` login shell launchd will use. The ADC probe reports an exit
// status only — the token value is discarded by the shell, never read here.
function runPreflight() {
  ...
  for (const binary of ['node', 'gcloud']) {
  ...
  const adc = shell('gcloud auth application-default print-access-token >/dev/null 2>&1');
  checks.push(['gcloud ADC', adc.status === 0
    ? 'ok — token printable from a login shell (value not read)'
    : 'UNAVAILABLE from a login shell; the scheduled run would fail']);
...
function runInstall(options) {
  if (!options.confirm) {
    fail([
      ...
      `  ${PLIST_PATH}`,
      '',
      'Preview it first:  node scripts/indexing-schedule.mjs print',
    ].join('\n'));
  }
  const { hour, minute } = resolveSchedule(options);
  ...
  process.stdout.write([
    `Installed ${LABEL} — daily at ${String(hour).padStart(2, '0')}:${String(minute).padStart(2, '0')}, runs the due-check loop.`,
    `plist  ${PLIST_PATH}`,
    `log    ${LOG_PATH}`,
    '',
    `Trigger one run now with:  launchctl kickstart -p gui/${process.getuid()}/${LABEL}`,
    'Remove it with:            node scripts/indexing-schedule.mjs uninstall',
  ].join('\n') + '\n');
}
...
function runHelp() {
  ...
  indexing-schedule.mjs status
  indexing-schedule.mjs preflight
  indexing-schedule.mjs print     [--hour H] [--minute M]
  indexing-schedule.mjs install   --confirm [--hour H] [--minute M]
  indexing-schedule.mjs uninstall

Defaults: --hour 7 --minute 30 (daily). Only install/uninstall change the
machine, and install requires --confirm.
`);
}
