...
You have access to **psi-swarm** — a CLI tool that runs Lighthouse many times against a URL across realistic device/network presets, then explains the results. It lives in the fleet-ops project at `foundry/tools/psi-swarm/` (this skill file is its canonical home).
...
psi-swarm lives in the fleet workspace at `~/Desktop/fleet/foundry/tools/psi-swarm/`.
The CLI binary is `node ~/Desktop/fleet/foundry/tools/psi-swarm/cli/dist/cli.js`.

If `cli/dist/cli.js` is missing, build it once:

```bash
cd ~/Desktop/fleet/foundry/tools/psi-swarm && pnpm install && pnpm --workspace cli run build
```
