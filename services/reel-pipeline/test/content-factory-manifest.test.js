...
test('strict Content Factory input rejects unapproved briefs before rendering', async () => {
  const fixtures = JSON.parse(await readFile('../../fleet-ops/test/fixtures/postiz/content-factory-v1.json', 'utf8'));
  let called = false;
...
test('every configured render engine has fixture-backed manifest normalization', async () => {
  const fixtures = JSON.parse(await readFile('../../fleet-ops/test/fixtures/content-factory/render-engine-manifests-v1.json', 'utf8'));
