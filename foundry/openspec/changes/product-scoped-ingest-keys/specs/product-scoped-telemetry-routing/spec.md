...
### Requirement: Polaris uses the App Health SDK on real routes

Polaris SHALL install the App Health Echo SDK with its product name, product key, and environment derived from runtime configuration. Integration verification SHALL use normal server startup plus an existing application route without adding a demo-only endpoint.
...
- **THEN** the product key routes its privacy-bounded SDK endpoint summary to `polaris / local`

#### Scenario: Staging Polaris deployment starts

- **WHEN** the merged staging deployment starts with `APP_ENV=staging`
- **THEN** its exported SDK endpoint summaries are routed to `polaris / staging` without a Polaris infrastructure change
