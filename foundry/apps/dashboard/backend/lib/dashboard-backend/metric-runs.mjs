...
function projectRoot(fleetRoot, project) {
  if (project.id === 'fleet-workspace') {
    return resolve(fleetRoot, 'foundry/apps/dashboard/web');
  }
...
function commandFor({ family, project, fleetRoot }) {
  ...
  if (family === 'ai') {
    return {
      command: process.execPath,
      args: [
        resolve(fleetRoot, 'foundry/apps/dashboard/backend/scripts/ai-visibility-canary.mjs'),
        '--project',
        project.id,
        '--fixture',
        resolve(fleetRoot, 'foundry/apps/dashboard/backend/test/fixtures/ai-visibility/providers-v1.json'),
      ],
...
function portfolioCommandFor({ family, fleetRoot, projects }) {
  ...
  if (family === 'psi') {
    ...
    return {
      command: process.execPath,
      args: [
        resolve(fleetRoot, 'foundry/apps/dashboard/backend/scripts/run-performance-portfolio.mjs'),
  ...
  if (family === 'search') {
    return {
      command: process.execPath,
      args: [
        resolve(fleetRoot, 'foundry/apps/dashboard/backend/scripts/search-console-collect.mjs'),
        '--discovery-cycle',
      ],
...
export function createMetricRunController({
  ...
  return {
    start({ family, projectId, scope = 'project' } = {}) {
      ...
      if (scope === 'project' && !project) fail('METRIC_PROJECT_INVALID', 'Unknown Dashboard project');
