# search-action-ledger Specification
...
read-only Search Console collector for the union of the complete canonical
public metric portfolio and all validated ten-root search targets, reports
progress, prevents duplicate active runs, and redraws the ledger after
successful completion.

#### Scenario: Operator updates Google Search evidence

- **WHEN** the operator activates Update and local mutation authentication is available
- **THEN** one portfolio Search collection starts for all canonical targets
- **AND** every validated ten-root target is included even when its catalog
  lifecycle excludes it from unrelated public metric pages
...
- **WHEN** local Google credentials or another collector prerequisite is unavailable
- **THEN** the control returns to its idle state
- **AND** the page reports the bounded failure without replacing existing evidence or exposing credentials
