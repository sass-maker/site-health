...
| Active | 23 | Supported and kept useful; improvements follow concrete needs. |
| Inactive | 36 | No planned development or autonomous expansion. Preserve relevant software, data and evidence. |
| Completed experiment | 1 | Finished and kept available; no active roadmap. |
| **Total** | **62** | Every canonical Fleet identity appears exactly once. |
...
| LaunchDesk (`launchdesk`) | Active | Give makers a provenance-honest catalog of launch destinations with per-product submission tracking. | Live publicly at sassmaker.com/launchdesk as a native catalog page in the shared SaaS Maker Pages site, fed by the generated dataset; the workspace app and live link-checker stay local-only by owner decision. | Keep: The 966-destination attributed catalog with explicit unknowns and browser-local tracking. Stop: Accounts, automated submission, outreach features and invented metrics. |
