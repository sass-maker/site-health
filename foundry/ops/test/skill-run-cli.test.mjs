...
test('teammate scorecard backfill is exactly 27 Codex and 7 Devin and idempotent', async () => {
  ...
  assert.equal(JSON.parse(second.stdout).duplicates, 34);

  const status = JSON.parse(run(root, ['status', '--json']).stdout);
  assert.equal(status.runCount, 34);
  ...
  assert.equal(prune.candidateCount, 34);
