...
`foundry/ops/config/spotlight-products.json`. The target map is in
`foundry/ops/config/spotlight-sync.json`.

Run the local check from the Fleet root:

```bash
node foundry/ops/scripts/sync-spotlight-products.mjs --check
```
