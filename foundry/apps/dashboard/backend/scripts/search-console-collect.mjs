...
const catalog = JSON.parse(readFileSync(resolve(FLEET_ROOT, 'foundry/apps/dashboard/backend/config/projects.json'), 'utf8'));
const config = JSON.parse(readFileSync(resolve(FLEET_ROOT, 'foundry/apps/dashboard/backend/config/search-console.json'), 'utf8'));
const rootBrands = JSON.parse(readFileSync(resolve(FLEET_ROOT, 'foundry/apps/dashboard/backend/config/root-brands.json'), 'utf8'));
const rootQueries = JSON.parse(readFileSync(resolve(FLEET_ROOT, 'foundry/apps/dashboard/backend/config/root-search-queries.json'), 'utf8'));
...
if (options.discoveryCycle) {
  const indexNow = spawnSync(
    process.execPath,
    [resolve(FLEET_ROOT, 'foundry/apps/dashboard/backend/scripts/indexnow-submit.mjs')],
    { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] },
  );
