...
- [x] 1.1 Retrieve current Cloudflare D1 import, SQL compatibility, binding, local-development, limits, and pricing documentation and record only constraints that affect the six projects.
...
## 8. Fleet convergence and optional retirement

- [x] 8.1 Update the Fleet cost-surface scanner, fixtures, registry validation, and spend governance so prepared, authoritative, and rollback-held database states remain distinct.
- [x] 8.2 Run strict OpenSpec validation, Fleet registry/tooling checks, per-project final checks, and `git diff --check`; record all skipped or blocked validation.
- [x] 8.3 Confirm all six project status files and Fleet registry identify current D1 authority without claiming Turso retirement prematurely.
- [x] 8.4 Present a final migration summary with per-project receipts, residual risks, retained Turso resources, and exact rollback status.
- [ ] 8.5 Request separate explicit approval for each Turso secret removal and database retirement; perform no deletion as an implicit migration task.
- [ ] 8.6 After all accepted cleanup ships, archive the OpenSpec change and record only durable shipped truth in Fleet and project status files.
