...
The 2026-07-17 GEO/SEO audit (`foundry/ops/docs/audit-report-geo-seo-2026-07-17.md`)
...
url, links) in `foundry/ops/config/agent-surfaces-registry.json`; we are just
...
- `apply-agent-surfaces.mjs` gains a JSON-LD generation + injection step:
  ...
  - For `opennext` products (JSX layouts — no safe text injection), emits the
    snippet to `foundry/ops/out/jsonld/<id>.html` for manual/agent-assisted
