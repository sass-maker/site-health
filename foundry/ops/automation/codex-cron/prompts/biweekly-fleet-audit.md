Run the weekly My Work product/design review from the Fleet workspace root.
...
Flow:
1. Read `foundry/ops/config/projects.json` and each in-scope project's `PROJECT_STATUS.md` before making recommendations.
...
4. Record at most 3 to 5 recommendations, updating existing project status or repository-native trackers instead of creating a parallel task database.
5. For slam-dunk unblocked tasks, dispatch independent work to available Fleet teammates with modest parallelism. Do not change auth, data model, pricing, secrets, deps, deployments, or product direction.
