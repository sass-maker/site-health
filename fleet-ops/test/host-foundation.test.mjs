...
function createFixture(t, secret = 'fixture-secret-value-should-never-appear') {
  ...
  const jobsFile = resolve(checkoutRoot, 'fleet-ops/automation/codex-cron/jobs.tsv');
  const systemJobsFile = resolve(checkoutRoot, 'fleet-ops/automation/codex-cron/system-jobs.tsv');
  const codexRunner = resolve(checkoutRoot, 'fleet-ops/scripts/agent-bin/run-codex-cron');
  const systemRunner = resolve(checkoutRoot, 'fleet-ops/scripts/agent-bin/run-system-cron');
  ...
  writeFileSync(
    systemJobsFile,
    `${SYSTEM_JOB_HEADER}\nnightly-sync\tyes\t15 2 * * *\t${secret}\t@fleet/fleet-ops/scripts/nightly-sync\n`,
  );
...
test('machine-local mutable paths are refused when they point inside the checkout', (t) => {
  ...
  const roleFile = fixture.writeRole('tracked-state', {
    leaseFile: resolve(fixture.root, 'checkout/fleet-ops/host/local-lease.json'),
  });

  const result = fixture.run('doctor', { roleFile });
  assert.equal(result.status, 1);
  assert.equal(result.json.error, 'TRACKED_PATH_REFUSED');
  assert.equal(existsSync(resolve(fixture.root, 'checkout/fleet-ops/host/local-lease.json')), false);
});
