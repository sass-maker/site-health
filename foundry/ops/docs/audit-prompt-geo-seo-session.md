...
1. Parallel directory spray (Playwright) for ~23 products × many directories;
   logs at foundry/ops/config/directory-submissions/{log.jsonl,status.json,products.json}.
...
7. IndexNow tooling added: foundry/ops/scripts/indexnow-submit.mjs +
   foundry/ops/config/indexnow.json + foundry/ops/docs/indexnow.md
   (Bing/Yandex/etc.; NOT Google).
8. Strategy docs: foundry/ops/docs/geo-dr-outcomes.md, directory-submissions.md.

## Hard rules (fleet)
- Read AGENTS.md / foundry/ops/docs/fleet-agent-standards.md if needed.
...
For every product in foundry/ops/config/agent-surfaces-registry.json:
...
highsignal.app, heypace.app, posttrainllm.com:
  bash foundry/ops/skills/seo-audit/scripts/seo-audit.sh <url> --site <origin>
Summarize FAIL/WARN only.

### C. IndexNow tooling correctness
Review foundry/ops/scripts/indexnow-submit.mjs + config/indexnow.json + docs/indexnow.md:
...
Run: node foundry/ops/scripts/indexnow-submit.mjs --check-keys
Run: node foundry/ops/scripts/indexnow-submit.mjs --dry-run --max 20
...
1) Live-check all products in foundry/ops/config/agent-surfaces-registry.json
   for /llms.txt, /api/ai, /index.md, sitemap (real XML not HTML).
2) Run agent-index-audit.mjs --all if available.
3) Review IndexNow script (foundry/ops/scripts/indexnow-submit.mjs): --check-keys,
